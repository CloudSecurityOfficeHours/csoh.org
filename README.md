...
| Resource | What it covers |
|---|---|
| 📝 [Meeting Recaps](https://csoh.org/meetings.html) | <!--count:meetings-->111<!--/count--> weekly session recaps, searchable |
...
| Resource | What it is |
|---|---|
| 🛡️ [Resources Directory](https://csoh.org/resources.html) | <!--count:resources_floor-->540+<!--/count--> tools, labs, CTFs, certifications - top-level nav link, auto-refreshed weekly |
...
Comprehensive catalog of **<!--count:resources_floor-->540+<!--/count--> cloud security resources** organized by 6 categories:
...
- **<!--count:meetings-->111<!--/count--> meeting recaps** with per-topic summaries and speaker notes
...
├── meetings.html               # Weekly meeting recaps (111 entries, topic-by-topic)
...
│
│  ── Catalogs & feeds ──
├── resources.html              # Main resource directory (540+ resources in 6 categories)
...
├── meetings/                   # 111 per-meeting recap pages (split from meetings.html)
...
- ✅ **`sitemap.xml`** - <!--count:sitemap_urls-->280<!--/count--> URLs, `<lastmod>` refreshed from git commit dates on every deploy ([tools/update_sitemap.py](https://github.com/CloudSecurityOfficeHours/csoh.org/blob/main/tools/update_sitemap.py))
...
- ✅ **Per-article social images** - <!--count:og_images-->264<!--/count--> unique 1200×630 JPG previews under `img/og/` (top-level pages via [tools/generate_og_images.py](https://github.com/CloudSecurityOfficeHours/csoh.org/blob/main/tools/generate_og_images.py), plus `img/og/breaches/`, `img/og/portfolio/`, and <!--count:meetings-->111<!--/count--> meeting recaps in `img/og/meetings/` via [tools/generate_meeting_og_images.py](https://github.com/CloudSecurityOfficeHours/csoh.org/blob/main/tools/generate_meeting_og_images.py)) so each page has its own LinkedIn/Slack/Twitter preview, not a generic site banner
