...
>
> Two follow-ups are gated on reading reports rather than on a date: DMARC `p=reject` (2b)
> and MTA-STS `mode: enforce` (6, after a clean seven-day TLS-RPT window). Both minimum
> windows have passed; action them once the reports look clean.

The sections that are done are kept as the record of what was changed and how to check
it, because that is the part that decays. Re-run the verification commands rather than
...
> ## Apply Terraform BEFORE you push
>
> Docs such as `how-csoh-org-is-secured.html`, `cloud-deployment.html`, `terraform.html`
> and `SECURITY.md` describe the security posture in the present tense. A change to them
> is true of the repository the moment it is committed, and true of production only when
> the corresponding `terraform apply` has run.
>
...
All four stacks plan clean; see 1c for the one AWS argument that must stay out to keep it
that way.

#### Before you start: two local traps that waste hours

Neither produces an error that points at its cause.
...
is an orphan and can be killed). Never reach for `-lock=false`: it does not clear the lock,
it starts a second concurrent apply against the same state, which is exactly the
corruption the lock exists to prevent.

### 1a. GCP - close the Workload Identity trust (DONE 2026-07-25)

The highest-value apply: without it, any workflow in the repo, on any branch, can mint
credentials for `csoh-deployer` (`roles/run.admin` + `roles/artifactregistry.writer`).
...
Verify two ways. The live config carries both claims:
...
and the service-account binding is a single `principal://.../subject/repo:...:environment:production`
...
More usefully, a normal deploy authenticating confirms the trust does not break the real
publish path. To confirm the gate actually
...
> `environment: production` (or, for the QA deployer on GCP only, `environment: qa`). That
> is a hard requirement on all three clouds, and it is deliberate.
...
runs. The complete set is tabulated in
...
Want `Location: https://csoh.org/about.html`. Anything starting `http://` is the redirect
loop (a 301 back to the URL just requested), and means the rule has been reverted or
overridden at the edge.
...
The apply adds `aws_cloudfront_response_headers_policy.security` and attaches it to
`aws_cloudfront_distribution.site`. Two other resources may show in a diff like it without
being drift, which is worth knowing before reading one:

- `aws_iam_role_policy.publisher` and `aws_s3_bucket_policy.site` can show as updating
...
**Do not add `viewer_certificate.minimum_protocol_version`.** With
`cloudfront_default_certificate = true`, CloudFront pins the viewer protocol to `TLSv1`
and ignores the argument, so the plan can never converge: the distribution sits
permanently in the diff and drags the two policy resources above with it. A stack that
always reports changes trains whoever runs it to skim the diff, which is precisely how
genuine drift goes unnoticed - the same failure shape as the Cloudflare `ignore_changes`
trap, from the other direction. It is not a TLS weakness: Cloudflare terminates the TLS
that visitors actually negotiate, and this distribution is only ever reached by
Cloudflare. Do not "fix" it by requesting an ACM certificate for a hostname nothing uses.
`infra/terraform/aws/cloudfront.tf` keeps the full reasoning in a comment, including the
one condition that makes it load-bearing: if this distribution ever moves to an ACM
certificate on a real hostname, add `minimum_protocol_version = "TLSv1.2_2021"` in the
same change.
...
Want `OK: all 8 headers match the repo across 1 request(s) to
https://<dist>.cloudfront.net/.` and, above it, `origins reached: aws=1`. Pass
`--samples 1` when pointing at a single origin: the default is 40 cache-busted requests, which only earns its cost against
...
> responses actually exercise the Cloudflare ruleset (AWS and GCP set the headers
...
**Current state**, as of 2026-07-26, from `dig +short TXT _dmarc.csoh.org` and
...
Why not `p=none`: with SPF and DKIM published correctly, `p=none` tells every receiving
mail server to deliver failures anyway, so anyone can send mail as `admin@csoh.org` and
have it land in inboxes. That address is the RFC 9116 contact in
`/.well-known/security.txt`, so a forged "send the PoC here instead" reply to a
vulnerability reporter is entirely plausible, and the site publishes a mailing list to a
large practitioner audience.
...
That second selector matters. DMARC passes if **either** SPF or DKIM aligns and passes - not
both. So a sender that signs with a valid DKIM signature for `csoh.org` survives
enforcement even though the SPF record lists only Google. That is what makes quarantine
low-risk here rather than a gamble on the newsletter.
...
The record is managed in Terraform, in
...
Deliberately *not* included: `adkim=s` / `aspf=s` (strict alignment is a separate tightening and should not ride along
with an enforcement change), and `ruf=` (forensic reports are ignored by most receivers
and can carry recipient PII).

> #### Import a hand-made record before the first apply
>
> A record created through the dashboard is unknown to Terraform, so a plain `apply`
> **creates a second one**. Cloudflare will happily hold two TXT records at the same name,
> and a domain publishing two DMARC records is treated by every receiver as publishing
> **none at all** - RFC 7489 says to ignore the domain's policy entirely rather than
> guess. That is strictly worse than `p=none`, with no error anywhere and a plan that looks
> like it worked.
>
> The `_dmarc` record was imported first (2026-07-25). The step: resolve the existing
> record's ID (needs the Terraform token from
> [README.md](README.md#there-are-two-cloudflare-tokens-and-they-are-not-interchangeable),
> which has DNS Edit),
>
> ```bash
...
> ```
>
> and confirm `plan` shows **1 to change, 0 to add** before applying.
>
> **The rule generalises (section 3 needs it too):** any record in this zone that was
...
Want exactly one line, containing `p=quarantine`. Two lines would mean an import was
...
The one part of section 2 still outstanding, deliberately: it waits for a clean reporting
period after quarantine went live on 2026-07-25. Change
...
Created via the Cloudflare dashboard's "add recommended CAA records" helper, then imported
into Terraform. Without CAA, every publicly trusted CA - roughly fifty - can issue for this
domain. HSTS preload does not help: a preloaded domain still trusts any publicly trusted
chain.

Eleven records are live, Cloudflare's full supported-CA set:
...
renewal was silently refused weeks later. (Having no `issuewild` at all is also valid,
and then `issue` covers wildcards too; `issuewild ";"`, which denies wildcards outright,
is never valid here.)
...
mirroring the live values exactly, and all eleven are in state. Records not yet in state
would be duplicated by any `terraform apply` in this stack; `dig +short CAA csoh.org`
returning eleven lines rather than twenty-two is the empirical confirmation that the
imports took.

**The general rule:** anything in this zone that was created through the dashboard has to
be imported before the first `apply` that touches the stack. That applies to any CAA
record added by hand, and the generator below is kept for exactly that case.
...
changes for any `caa_` resource**. If it wants to
create them, the import did not take - stop, because applying then doubles every record.

> ### Do NOT add `issuewild ";"`
>
> Nothing here needs a wildcard by name, which makes denying wildcards look safe. It would
> break certificate renewal and eventually take TLS down.
>
...
Want eleven lines: five `issue`, five `issuewild`, one `iodef`.

**Then verify again after each renewal.** CAA is enforced at issuance, so a mistake here
surfaces at the next renewal, not at apply:

```bash
openssl s_client -connect csoh.org:443 -servername csoh.org </dev/null 2>/dev/null | openssl x509 -noout -issuer -dates
```

A `notAfter` that has moved forward means renewal succeeded under the CAA policy. If
...
>
> Signing AND delegation are complete. As of 2026-08-09:
>
> | Check | Result |
> |---|---|
...
>
> **DO NOT SUBMIT A DS RECORD.** One is published. Submitting again, or submitting a DS
> for a key that is not the current KSK, is the single DNSSEC failure mode that takes a
> domain offline for every validating resolver - the domain does not slow down, it
> vanishes. If you are reading this looking for something to do, there is nothing here.
>
> **Two facts that are easy to get wrong.**
>
> First: `cloudflare_zone_dnssec` enables **signing** only. Delegation is a separate
> manual step, and Cloudflare being the registrar does not make it automatic.
>
> Second: **the `ad`-flag check is unreliable on some networks.** Running
> `dig +dnssec csoh.org A @1.1.1.1 | grep 'flags:'` returns no `ad` flag from at least
> one network path here, and it does the same for KNOWN-GOOD signed domains
> (`cloudflare.com`, `internetsociety.org`) - the AD bit is being stripped in transit.
> Read as a verdict, that false negative says "delegation never happened". Do not trust
> it. Ask a resolver that reports validation over HTTPS instead:
>
> ```sh
...
With delegation live, both are protected: a forged answer fails validation rather than
being accepted.

### Signing and delegation are two separate things, and both are done
...
1. **Signing**, at the DNS provider. Cloudflare generates the keys, publishes `DNSKEY`, and
   ...
   it is on. The resource carries `prevent_destroy = true`, so a stray
...
2. **Delegation**, at the registry. The `DS` record - a hash of the KSK - has to be
   ...
   down from the root has a reason to trust our `DNSKEY`. **This is a separate step, and
...
**Cloudflare being both registrar and DNS provider does not merge those two steps.**
`whois csoh.org` does report `Registrar: Cloudflare, Inc.`, and the nameservers are
Cloudflare's, but the DS submission is still a distinct action someone has to take.
`terraform apply` does not do it and waiting does not do it.
...
missing DS is the safe direction; a wrong DS is the dangerous one, and with a DS live that
is the failure mode to protect against.
...
It returns no `ad` flag on at least one network path here even for known-good signed
zones - `cloudflare.com` and `internetsociety.org` fail it identically, because the AD bit
is stripped in transit. It measures the path, not the zone. Ask a resolver that reports its validation result
...
> stale-DS outage described above. DS 2371 is published at `.org`, so a transfer done
> without this step can take the domain offline for every validating resolver.

---

## 5. Turn off Cloudflare's managed robots.txt injection (DONE 2026-07-26)

Cloudflare's managed `robots.txt` prepends its own AI-crawler block ahead of the repo's
file: a `Cloudflare Managed` section that `Disallow`s several crawlers the repo's own
`robots.txt` and `llms.txt` explicitly allow. Not a vulnerability, but the same failure
class as the Terraform `ignore_changes` trap: the edge silently overriding what is in Git.

**The repo's position is kept** (crawlers allowed): the managed `robots.txt` is disabled at
Cloudflare dashboard → **Security → Bots → AI Crawl Control** (2026-07-26), so
`robots.txt` and `llms.txt` in the repo remain the authority. The alternative would be to
keep Cloudflare's position and delete the AI-crawler `Allow` section from `robots.txt`,
correcting the corresponding sentence in `llms.txt`.
...
Want `ok  99 lines match, byte-for-byte after whitespace normalization` (the line count
tracks the file).

That checker is a deploy gate: the `purge-cloudflare` job in `deploy.yml` runs
...
Without these records, SMTP between mail servers uses opportunistic TLS, so an on-path
attacker can strip `STARTTLS` and inbound mail is delivered in cleartext. Since `admin@csoh.org` is the published
...
3. `https://mta-sts.csoh.org/.well-known/mta-sts.txt`, served as `text/plain`:
   ...
   The hostname needs its own HTTPS endpoint, but not a Worker: `mta-sts` is a proxied CNAME onto the apex, so it is
   ...
   wildcard (`*.csoh.org`), so the certificate is valid for the hostname and nothing in
   `dns_caa.tf` needs to change. `dns_mail.tf` records why the `www` redirect does
...
move to `mode: enforce` is outstanding: confirm the TLS-RPT reports are clean for at
...
Independently of which you pick, the **GCP billing budget alert**
(`infra/terraform/gcp/budget.tf`) is what notices sustained direct traffic to Cloud Run.

---

## Verification sweep

Re-run the whole set rather than trusting the statuses above:
