...
Keep both lists complete. A page in neither is silently never visited - no error, no warning, just a page shipping with zero glossary links. `tools/check_crosslink_coverage.py` fails if a root page is in neither list, and runs in `validate-html.yml`.
...
Both import `derive_keys`, `slugify` and the denylists from [`glossary_terms.py`](glossary_terms.py), and use the same `glossary-link` class so they look identical in the DOM. Keep that logic in the shared module rather than copying it into either script, or the two drift apart. The one thing they do *not* share is the denylist: `crosslink_pages.py` uses `PAGE_DENYLIST` (`BASE_DENYLIST` plus words like `data`, `policy`, `account` that recur constantly in page prose), while the glossary uses the shorter `BASE_DENYLIST`.
