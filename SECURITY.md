...
- Cleanup policy keeps the most recent 30 versions (Git raises this to 50, pending the apply below). A `DELETE` rule for untagged versions older than 7 days has been live since the repo was created and can never fire - `immutable_tags=true` means every image is tagged at birth, so nothing becomes untagged. A second `DELETE` rule for **tagged** versions older than 30 days is committed but **not yet applied** (`73f884db`, 2026-08-25); until that apply lands the repository still grows without bound.
